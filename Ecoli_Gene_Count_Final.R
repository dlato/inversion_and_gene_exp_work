...
norm <- rename(as.data.frame(norm), c('norm1'='ATCC2_count', 'norm2'='ATCC3_count'))


ATCC_GSE94978 %<>% cbind(norm) %>% group_by(gene_id) %>%
  mutate(count = median(c(norm1, norm2)),
         ATCC_GSE94978 = 1) %>% dplyr::select(-c(V13))
...
BW25113_GSE73673 %<>%
  ...
  dplyr::select(gene_id, norm_count, raw_count, BW25113_GSE73673) %>% arrange()
...
BW25113_GSE85914 %<>% 
  ...
  dplyr::select(Name, gene_id, norm_count, raw_count, BW25113_GSE85914) %>% arrange()
...
BW25113 %<>%
  ...
  ungroup() %>%
  dplyr::select(gene_id=Name, Locus_tag, norm_count, raw_count, 
...
K12_DH10B_GSE98890 %<>% group_by(gene_name) %>%
  ...
  ungroup() %>%
  dplyr::select(gene_id, locus_tag, Control_rep1, Control_rep2)
#all unique now 
...
K12_MG1655_GSE54199 %<>% group_by(gene_id.x, Locus_tag) %>%
  ...
  dplyr::select(Locus_tag, gene_id = gene_id.x, Translation_start = Translation_start.x,
...
K12_MG1655_GSE60522 %<>% cbind(norm) %>%
  ...
  dplyr::select(gene_id, norm_count, raw_count, K12_MG1655_GSE60522)
...
K12MG1655 <- merge(K12_MG1655_GSE60522, 
  ...
  dplyr::select(-c(raw_count.x, raw_count.y, norm_count.x, norm_count.y)) %>% #4573
  ...
  mutate(norm3 = norm, raw3 = raw_norm) %>% dplyr::select(-c(norm, raw_norm)) %>%
  merge(K12_MG1655_GSE40313, by = "Locus_tag", all = TRUE) %>% 
  mutate(norm4 = norm_count, raw4 = raw_count) %>% 
  dplyr::select(-c(norm_count, raw_count)) %>% #4814
...
K12MG1655 %<>% group_by(unique_id) %>% 
  ...
  ungroup() %>%
  dplyr::select(Locus_tag, gene_id, Strand, RPKM, Translation_start, Translation_stop,
...
H7_GSE46120 %<>% group_by(gene_id) %>%
  ...
  dplyr::select(gene_id, V1, V2, V3, V4, V5, V7, raw_count, norm_count,H7_GSE46120_36,H7_GSE46120_37)
...
#ATCC_info_old <- fread("../Genomes/Ecoli_ATCC_25922_NZ_CP009072_13May20_gene_info.txt")
BW_info <- fread("../Genomes/Ecoli_BW25113_NZ_CP009273_gene_info.txt")
#BW_info_old <- fread("../Genomes/Ecoli_BW25113_NZ_CP009273_13May20_gene_info.txt")
H7_info <- fread("../Genomes/Ecoli_0157H7_chrom_Sakai_BA000007_gene_info.txt")
#H7_info_old <- fread("../Genomes/Ecoli_0157H7_chrom_Sakai_NC_002695_3Mar20_gene_info.txt")
K12_DH_info <- fread("../Genomes/Ecoli_K12_DH10B_NC_010473_gene_info.txt")
#K12_DH_info_old <- fread("../Genomes/Ecoli_K12_DH10B_NC_010473_13May20_gene_info.txt")
K12_MG_info <- fread("../Genomes/Ecoli_K12_MG1655_chrom_U00096_gene_info.txt")
#K12_MG_info_old <- fread("../Genomes/Ecoli_K12_MG1655_chrom_U00096_13May20_gene_info.txt")
...
ATCC_GSE94978_final <- ATCC_GSE94978 %>% merge(ATCC_info, by.x="gene_id",
                                               by.y="gbk_locus_tag") %>%
  dplyr::select(gene_id, gbk_start, gbk_end, gbk_midpoint, gbk_gene_id, gbk_old_locus_tag,
                norm_exp=count)
...
# BW25113_names <- BW25113 %>% dplyr::select(gene_id, Locus_tag) #4507
# K12MG1655_names <- K12MG1655 %>% dplyr::select(gene_id, Locus_tag) #4898
...
# 
# #check if BW_info 
# BW_info_names <- BW_info %>% dplyr::select(key, gbk_gene_id) %>%
...
#   dplyr::select(Locus_tag, gene_id, gbk_gene_id) 
...
BW25113_final <- BW25113_comp[is.na(BW25113_comp$gbk_start),] %>% dplyr::select(-c(gbk_start:gbk_strand)) %>%
  merge(BW_info, by.x = "gene_id", by.y = "gbk_gene_id", all.x = T) %>% mutate(key=key.x,
                                                                               gbk_gene_id=gene_id) %>%
  dplyr::select(-c(key.y,key.x)) 
...
BW25113_final %<>% dplyr::select(gene_id, Locus_tag, gbk_start, gbk_end, gbk_midpoint, gbk_gene_id, gbk_locus_tag,
                norm_exp=norm_count)
...
K12_DH10B_final <- K12_DH10B_GSE98890 %>% merge(K12_DH_info, by.x="locus_tag", 
  ...
  dplyr::select(locus_tag, gene_id, gbk_start, gbk_end, gbk_midpoint, gbk_gene_id, gbk_locus_tag,
                norm_exp=norm_count)
...
K12MG_final %<>% merge(K12_MG_info, by.x="Locus_tag", by.y="gbk_locus_tag", all.x=T) %>%
  dplyr::select(Locus_tag, gene_id, gbk_start, gbk_end, gbk_midpoint, gbk_gene_id, gbk_old_locus_tag,
                norm_exp=norm)
#3891
...
#   dplyr::select(-c(Start, Stop, Midpoint, gene_id.y, Strand.y)) %>%
